...
export default function Hero() {
  ...
  const waUrl =
    'https://wa.me/270126430131?text=Hi%20Phago%20Media%2C%20I%20would%20like%20to%20discuss%20a%20booking.';

  return (
    <section
      id="hero"
      className={`relative h-screen min-h-[600px] overflow-hidden ${
        revealed ? 'shutter-reveal' : ''
      }`}
    >
      {heroImages.map((img, i) => (
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(to bottom, rgba(27,42,74,0.75) 0%, rgba(27,42,74,0.5) 50%, rgba(27,42,74,0.85) 100%)',
            }}
          />
        </div>
      ))}
      ...
      <div className="absolute top-24 right-20 flex items-center gap-2 z-10">
        ...
        <span className="text-white/60 font-inter text-xs tracking-widest">
          REC
        </span>
      </div>
        ...
        <div className="max-w-3xl">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              ...
              className="px-8 py-4 border border-white/20 text-white font-grotesk font-semibold text-sm tracking-[0.15em] hover:bg-white/10 transition-all duration-300 text-center"
            >
              VIEW SERVICES
            </button>
          </div>
        ...
        {heroImages.map((_, i) => (
          <button
            ...
            className={`transition-all duration-300 ${
              i === currentImg
                ? 'w-6 h-1 bg-signal-red'
                : 'w-1 h-1 bg-white/40 rounded-full'
            }`}
          />
        ))}
      </div>
      ...
      >
        <span className="font-inter text-xs tracking-widest rotate-90 mb-2">
          SCROLL
        </span>
